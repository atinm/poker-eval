...
int DoEval(int64_t IDin)
{
	...
	int holdrank = low ? LowHandVal_NOTHING : 0;
	int workcards[8];  // intentially keeping one as a 0 end
	int holdcards[8];
	int numevalcards = 0;
    CardMask cards;
	...
	{	 // if I have a good ID then do it...
		...
		{  
            ...
            //printf("\nmaking card rank %d, suit %d, card: ", rank, suit-1);
            StdDeck_CardMask c;
            //StdDeck_CardMask_RESET(c);
            c = StdDeck_MASK(StdDeck_MAKE_CARD(rank, (suit - 1)));
            //StdDeck_printMask(c);
            //printf("\ncards: ");
            StdDeck_CardMask_OR(cards, cards, c);
		...
		switch (numevalcards) {  // run Cactus Keys routines
			...
			case 5 : 
                if (low)
                    holdrank = Hand_EVAL_LOW8(cards, 5);
                else
                    holdrank = Hand_EVAL_N(cards, 5);
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i <= 9; i++) { // display the results
        if (i == 0 && low)
            printf("\n%16s = %d", "Ineligible", handTypeSum[i]);
        else
